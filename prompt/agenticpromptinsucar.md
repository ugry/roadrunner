...
- FULL PHASE-1 (not deferred): multi-line services, B2B white-label multi-tenancy, multi-cloud
  warm-standby, chaos/DR game-days, 24/7 on-call, pentest, GDPR legal review, load tests,
...
- CONSUMER app (public, discoverable): register/login (Keycloak CUSTOMER realm), manage policies/
...
- OPERATOR/COORDINATOR console (SEPARATE, NON-discoverable host/path; STAFF realm; MFA; zero-trust;
...
- PARTNER portal (B2B white-label): partners configure branding, service lines, coverage products,
  view their tenants' cases/KPIs, manage their own provider preferences. Separate realm.
...
- Auth: Keycloak — CUSTOMER, STAFF, PARTNER realms; OIDC/OAuth2/SAML/Kerberos/AD; RBAC roles
...
- Observability: Prometheus + Loki + Grafana + Alertmanager; OpenTelemetry; PagerDuty on-call.
- Primary cloud AWS; portable (containers + IaC). WITHIN region: active-active + auto node-failover.
  FULL PHASE-1: cross-cloud warm-standby.
...
- Surface isolation: consumer / operator / partner apps = separate hosts + realms + cookies;
...
- HA (NO single point of failure): quorum survives one-node loss -> ODD member counts (min 3, or
  2 data + witness) across >=3 AZs. K8s control plane 3 AZs; Postgres Patroni + etcd quorum (sync
  replication, auto-failover, floating endpoint); NATS R3; Redis Sentinel quorum; stateless services
  >=2 replicas + podAntiAffinity/topologySpread. Do NOT ship a quorum-less 2-node stateful cluster.
...
7. Keycloak (customer/staff/partner realms) with RBAC incl. product_owner + partner_admin.
8. Quorum HA cluster (3 / 2+witness across >=3 AZs) with proven node-failover.
...
1. Repo + CI + Terraform (dev account) + quorum HA cluster + `make dev-up` bootstrap.
2. REAL vertical slice (no mocks): live Connect + Streams->BFF->console screen-pop + tenants/RLS +
   real-seeded Postgres (Patroni) + Keycloak realms + one real tow connector + Pinpoint SMS.
...
- SSO across customer/staff/partner realms; RBAC enforced; 3-case IAM holds; every prod access grant
...
- Read/inspect before modifying. No destructive action without approval + inventory (except
  ...
  an exposed key. Prefer minimal, portable, memory-safe designs. Keep the human coordinator in the loop.
